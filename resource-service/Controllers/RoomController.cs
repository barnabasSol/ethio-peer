...
public class RoomController(RoomRepository roomRepo) : ControllerBase
{
    ...
    //Get Rooms
    [HttpGet]
    public async Task<IActionResult> GetRooms([FromQuery] string? memberId,CourseCategory? category,[FromQuery] bool explore)
    {
        try
        {
            if (!string.IsNullOrEmpty(memberId))
            {
                var rooms = await _roomRepo.GetRoomsByMemberId(memberId,explore);
                return Ok(rooms);
            }
            else
            {
    ...
    [HttpGet("suggestions")]
    public async Task<IActionResult> GetSuggestedRooms([FromQuery] List<string>? courses,[FromQuery] string memberId )
    { try
        {
            var rooms = await _roomRepo.GetSuggestedRooms(courses, memberId);
            return Ok(rooms);
        }
        catch (ArgumentNullException e)
        {
    ...
    public async Task<IActionResult> AddParticipant([FromBody] RoomMemberDTO roomMember)
    {
       try
       {
           if (string.IsNullOrEmpty(roomMember.MemberId)|| roomMember.RoomId == Guid.Empty) return BadRequest("Please provide a valid parameter");
